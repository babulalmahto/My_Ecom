...
function CreateCategory() {
    ...
    async function deleteCategoryHandler(id) {
        try {
            let result = await axios.delete(`/api/ecom/delete-category/${id}`, { headers: { "Authorization": auth.token } })
    ...
    async function sumbmitCategoryHandler() {
        try {
            let result = await axios.post(`/api/ecom/create-category`, { name: category }, { headers: { "Authorization": auth?.token } })
            if (result.data.success) {
                toast(result.data.message)
                setCategory('')
    ...
    const handleOk = async () => {
        try {
            let { data } = await axios.put(`/api/ecom/update-category/${categoryId}`, { name: category }, { headers: { "Authorization": auth?.token } })
            if (data.success) {
                toast(data.message)
                setChangeCategory(!changeCategory)
                setCategory('')
